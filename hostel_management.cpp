...
private:
    ...
    void commitStudentID(int newID);
    int totalclothes;
    double ratePerCloth;
    int bedNumber;
    string loggedInUser="";
...
while(!isValidContact(family_contact));
    // Prompt for room type until a valid value is entered (case-insensitive)
    ...
    do {
        ...
        transform(rt.begin(), rt.end(), rt.begin(), [](unsigned char c){ return tolower(c); });
        if(rt == "single") { roomtype = "Single"; validRoom = true; }
        else if(rt == "double") { roomtype = "Double"; validRoom = true; }
        else if(rt == "triple") { roomtype = "Triple"; validRoom = true; }
        else { cout << "Invalid room type! Please enter Single, Double or Triple.\n"; }
    } while(!validRoom);

    // Prompt for bed number until a valid, unoccupied bed is chosen
    bool bedSelected = false;
    do {
        cout << "Choose bed number (";
        if(roomtype == "Single") cout << "1-" << singleBeds << "): ";
        else if(roomtype == "Double") cout << "1-" << doubleBeds << "): ";
        else cout << "1-" << tripleBeds << "): ";

        if(!(cin >> chosenBed)) {
            // handle non-integer input
            cout << "Invalid bed number! Please enter a numeric value.\n";
            cin.clear();
        ...
        if(roomtype == "Single") {
            if(chosenBed < 1 || chosenBed > singleBeds) {
                cout << "Invalid bed number! Please choose between 1 and " << singleBeds << ".\n";
            } else if(singleRoomOccupancy[chosenBed-1] == 1) {
                cout << "Bed already occupied. Choose another bed.\n";
            } else {
                singleRoomOccupancy[chosenBed-1] = 1;
                bedNumber = chosenBed;
                bedSelected = true;
            }
        }
        else if(roomtype == "Double") {
            if(chosenBed < 1 || chosenBed > doubleBeds) {
                cout << "Invalid bed number! Please choose between 1 and " << doubleBeds << ".\n";
            } else if(doubleRoomOccupancy[chosenBed-1] == 1) {
                cout << "Bed already occupied. Choose another bed.\n";
            } else {
                doubleRoomOccupancy[chosenBed-1] = 1;
                bedNumber = chosenBed;
                bedSelected = true;
            }
        }
        else { // Triple
            if(chosenBed < 1 || chosenBed > tripleBeds) {
                cout << "Invalid bed number! Please choose between 1 and " << tripleBeds << ".\n";
            } else if(tripleRoomOccupancy[chosenBed-1] == 1) {
                cout << "Bed already occupied. Choose another bed.\n";
            } else {
                tripleRoomOccupancy[chosenBed-1] = 1;
                bedNumber = chosenBed;
                bedSelected = true;
            }
        }
...
else
{
    ...
    // Generate ID now (peek), ensure uniqueness, then commit the counter and append record
    string id = generateStudentID();
    ifstream checkFile2("students.txt");
    string existingLine2;
    string existingID2;
    bool idExists = false;
    while(getline(checkFile2, existingLine2)) {
        if(existingLine2.empty()) continue;
        string _name, _contact, _fname, _fcontact, _rtype, _food;
        int _bed;
        parseStudentRecord(existingLine2, existingID2, _name, _contact, _fname, _fcontact, _rtype, _bed, _food);
        if(existingID2 == id) {
            idExists = true;
            break;
        }
    ...
    int numericID = 0;
    try {
        numericID = stoi(id.substr(3));
    } catch(...) {
        cout << "Error parsing generated ID.\n";
        file.close();
        return;
    }
    commitStudentID(numericID);
...
string hostel::generateStudentID()
{
    ...
    fin.close();
    int newID = last + 1;
    // peek only: do not write to file here
    return "STU" + to_string(newID);
}

void hostel::commitStudentID(int newID)
{
    ofstream fout("id_counter.txt", ios::trunc);
    if(!fout) {
        cout << "ERROR: Cannot update id_counter.txt\n";
        return;
    }
    fout << newID;
    fout.close();
